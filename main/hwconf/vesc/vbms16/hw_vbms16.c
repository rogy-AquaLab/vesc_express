...
static uint16_t m_fet_state_ic1 = 0;
...
static void bq_init(uint8_t dev_addr) {
	...
	subcommands_write8(BQ_ADDR_1, FET_CONTROL, m_fet_state_ic1);
...
static lbm_value ext_get_vchg(lbm_value *args, lbm_uint argn) {
	...
	bool ok = false;
	int16_t res = command_read(BQ_ADDR_1, PACKPinVoltage, &ok);
	if (!ok) {
		lbm_set_error_reason(error_comm_bq1);
		return ENC_SYM_EERROR;
	}
...
static lbm_value ext_set_out(lbm_value *args, lbm_uint argn) {
	...
	if (subcommands_write8(BQ_ADDR_1, FET_CONTROL, m_fet_state_ic1)) {
		return ENC_SYM_TRUE;
	} else {
		lbm_set_error_reason(error_comm_bq1);
		return ENC_SYM_EERROR;
...
void hw_init(void) {
	...
	gpconf.pin_bit_mask = BIT(PIN_PCHG_EN) | BIT(PIN_COM_EN);
